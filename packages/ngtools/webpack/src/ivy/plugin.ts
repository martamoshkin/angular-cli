...
import { findResources } from '../transformers/find_resources';
...
export class AngularWebpackPlugin {
  ...
  private updateAotProgram(
    ...
    // Create an ngtsc plugin instance used to initialize a TypeScipt host and program
    ...
    const getDependencies = (sourceFile: ts.SourceFile) => {
      // TODO: Investigate alternatives to this process.
      //       This currently needs to walk the AST and be kept in sync with AOT functionality.
      return (
        findResources(sourceFile)
          // resourceNameToFileName is always added by this plugin
          // tslint:disable-next-line: no-non-null-assertion
          .map((dep) => host.resourceNameToFileName!(dep, sourceFile.fileName) as string)
          .filter((dep) => !!dep)
          .reduce<string[]>(
            (deps, resource) =>
              deps.concat(resource, ...resourceLoader.getResourceDependencies(resource)),
            [],
          )
      );
    };
